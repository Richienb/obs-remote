...
<panel-wrapper :content-class="['panel-mixer']">
    ...
    <div class="audio-devices" v-if="audioDevices">
        <button :id="audioDevice.name.replace(/\W/g,'-')" :runme="getMuteStatus(audioDevice.name)" class="audio-device" v-for="audioDevice in audioDevices" :key="audioDevice.name" @click="toggleMuteForAudioDevice(audioDevice.name)"
