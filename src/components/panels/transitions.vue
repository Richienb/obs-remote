...
<panel-wrapper :content-class="['panel-transitions']">
    ...
    <div class="transitions" v-if="allTransitions">
        <button class="transition" v-for="transition in allTransitions" :key="transition.name" :class="[transition.name === currentTransition ? 'is-active' : 'is-inactive']" @click="switchTransitions(transition.name)"
