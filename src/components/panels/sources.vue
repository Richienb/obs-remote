...
<panel-wrapper :content-class="['panel-sources']">
    ...
    <div class="sources" v-if="currentScene">
        <button class="source" v-for="source in currentScene.sources" :key="source.name" :class="[source.render ? 'is-active' : 'is-inactive']" @click="setRender({scene: currentScene.name, source: source.name, render: !source.render})">
            {{ source.name }}
        </button>
    </div>
    <div v-else>
