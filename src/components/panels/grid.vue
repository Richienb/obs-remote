<template>
<panel-wrapper :content-class="['panel-grid', this.directionClass]" :is-grid="true" @mouseleave.native="stopResizing" @mousemove.native="handleResize" @mouseup.native="stopResizing" @touchend.native="stopResizing" @touchmove.native="handleResize">
    ...
    <template v-for="(panel, id, index) in childPanels">
        <div v-if="editing && index > 0" class="resize-handler" draggable="true" @mousedown.prevent="startResize($event, index - 1)" @touchstart.prevent="startResize($event, index - 1)" :key="id + '-resize'">
            <span class="dot"></span>
            <span class="dot"></span>
            <span class="dot"></span>
        </div>
        <panel ref="panels" :key="id" :panel="panel" :id="id" :depth="depth + 1" :style="{flexBasis: (weights[index] || 1) * 100 / childCount + '%'}" :data-basis="childCount" />
    </template>
    <!-- adding in edit mode -->
    ...
    <div slot="settings">
        ...
        <button :class="{'is-inactive': this.settings.direction !== 'row'}" @click="setSetting('direction', 'row')">
            <i class="material-icons-outlined md-48">view_column</i>
        </button>
        <button :class="{'is-inactive': this.settings.direction !== 'column'}" @click="setSetting('direction', 'column')">
            <i class="material-icons-outlined md-48">view_stream</i>
        </button>
    </div>
</panel-wrapper>
</template>
