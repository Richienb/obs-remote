...
<panel-wrapper :content-class="['is-scrollable', 'panel-scenes', 'per-row-' + perRow]">
    ...
    <button class="scene" v-for="scene in scenes" :key="scene.name" :class="[scene.name === currentScene ? 'is-active' : 'is-inactive']" @click="switchScenes(scene.name)">
        {{ scene.name }}
    </button>
    ...
    <div slot="settings">
        ...
        <label>
            Transition time
            <div class="input-group">
                <input v-model.number="transitionSeconds" type="number" min="0" step="0.1">
            </div>
        </label>
    </div>
</panel-wrapper>
