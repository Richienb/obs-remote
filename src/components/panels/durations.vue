...
<panel-wrapper :content-class="['panel-durations']">
    ...
    <div class="durations" v-if="allDurations">
        <button class="duration" v-for="duration in allDurations" :class="[duration === currentDuration ? 'is-active' : 'is-inactive']" :key="duration" @click="changeDuration(duration)">{{ duration }}ms</button>
    </div>
</panel-wrapper>
</template>
